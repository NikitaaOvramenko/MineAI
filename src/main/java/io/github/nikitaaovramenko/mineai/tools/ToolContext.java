...
public record ToolContext(MinecraftServer server, UUID playerId) {
    ...
    public boolean canUseCommand(String name) {
        CommandNode<CommandSourceStack> command = server.getCommands().getDispatcher().getRoot().getChild(name);
        return command != null && command.canUse(player().createCommandSourceStack());
    }
}
